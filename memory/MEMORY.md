...
- Prioridad inmediata: validar en el preview nuevo el delete optimista en `/hub` y el nuevo flujo guiado de `Abrir acta y luego PDF`.
- Luego, en el mismo preview, revalidar `Seleccion`, `Evaluacion`, `Contratacion`, `Sensibilizacion` y `Condiciones Vacante` con foco en autosave por keystroke, blur, cambio de pestaña, cierre y seed inicial de borrador local.
- Finalmente, en el mismo preview, revalidar gate liviano, lookup por cedula, repetidores shared y la ausencia de mensajes engañosos persistentes.

1. Crear un preview nuevo que incluya el fix de delete optimista del hub y el flujo guiado de `Abrir acta y luego PDF` sobre la base actual.
2. En ese QA, validar `/hub` confirmando desaparicion inmediata del draft, rollback correcto si falla el backend y lectura de timings del route si la sensacion de lentitud persiste.
3. En el mismo QA, validar que el CTA combinado abre el acta de forma confiable y guia al usuario al PDF sin dejar mensajes engañosos, y luego retomar el smoke agrupado de autosave, gate, lookup y repetidores.
