...
- Follow-ups acordados en plan v2 (pendientes de abrir como issues `ods` + `tech-debt`): #113 sibling de #73 (RPC `form_finalization_request_lookup_by_artifact` para reemplazar admin queries directas con multiples filtros JSON), #75 telemetry `ods_import_failures` para reemplazar `console.warn` server-side por persistencia auditada, #82 actor_user_id en `ods_motor_telemetria`, #108 tests integration con Supabase real para todos los RPCs criticos, #115 cleanup 17 lint warnings preexistentes (16 unused vars + 1 react-hooks/exhaustive-deps en Seccion4 como fix defensivo).
