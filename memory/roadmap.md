...
- Tech-debt #113 sibling de #73 cerrado via PR #119 (commit f9a01a1): RPC `form_finalization_request_lookup_by_artifact(p_artifact_kind, p_artifact_id, p_artifact_url)` con `security definer set search_path = ''` retorna jsonb `{rows:[...]}` cap 16; single OR query con 11 filter paths (3 sheet + 8 drive); rewrite TS de `resolveArtifactActaRef` usa `admin.rpc(...)` en lugar de `Promise.all` con 3-8 queries directas; `FINALIZATION_ARTIFACT_SELECT` eliminado; mantiene dedupe TS-side + `rowMatchesArtifact` + `rows.length === 1`. Tests integration gated reales contra remoto: 7 passed con synthetic auth user (cleanup robusto en `afterAll`: fixtures por idempotency_key + auth user). No-fantasma textual sobre archivo de migracion. Migracion remota aplicada con autorizacion del PO; `has_function_privilege` confirmo `anon=false, authenticated=false, service_role=true`. **Cierra el patron RPC server-only en `/api/ods/importar`**: post-#73+#113, ya no hay admin client directos a tablas en este endpoint.
- Tech-debt #108 cobertura integration RPCs ODS cerrado via PR #120 (commit a2f6d7c): auditoria + closure de gaps + doc operativo. Nuevo `envelopeCodes.integration.test.ts` con 3 tests cubriendo 10 escenarios (early envelopes record/finalize + guards post-link). Doc nuevo `docs/ods_integration_tests.md` con matriz completa RPC × test file × casos cubiertos (7 RPCs/contratos + 5 tests telemetry complementarios), env vars requeridas/opcionales, comandos PowerShell con loader `.env.local`, politica explicita "todo RPC ODS server-only nuevo nace con integration test gated". Helpers SQL puros excluidos por contrato (parity TS/SQL ya cubre). `dedupe_conflict` excluido como excepcion consciente (race recovery). Suite ODS completa contra remoto: 388 passed + 9 skipped por JWTs opcionales. Indexado en `MEMORY.md` "Canonico minimo".
- Tech-debt #115 cleanup 20 lint warnings cerrado via PR #121 (4 commits split por area: eslint config + ODS + Seguimientos + Seccion4 fix defensivo). Config eslint con `argsIgnorePattern`/`varsIgnorePattern` `^_` silencia `_request`/`_supabase` sin tocar codigo. Eliminadas 16 imports/vars muertos cross-modulo (e2e ODS, ImportPreviewDialog, Seccion4 `rowErrors` write-only, seguimientosRouteActor test scaffolding F0-F4, SeguimientosCaseOverview/CaseTimeline/FollowupStageEditor imports muertos). Fix defensivo `react-hooks/exhaustive-deps` en `Seccion4.tsx:139` copiando `debounceRefs.current` a variable local antes del cleanup. Sin tocar `SeguimientosBaseStageEditor.tsx` (out of scope, cubierto por #116). `npm run lint` post-PR reporta exactamente 2 warnings (#116). **Sweep tech-debt ODS completo**: 6 de 7 tech-debts cerradas en 2 dias (#74 schema-drift, #73 RPC formato_finalizado, #75 import_failures, #82 actor_user_id, #113 RPC form_finalization sibling, #108 cobertura integration, #115 lint cleanup). Pendiente unico fuera del milestone: #116 (otro PO).
- Follow-ups acordados (pendientes): #116 investigacion PO Seguimientos sobre warnings funcionales en `SeguimientosBaseStageEditor`.
