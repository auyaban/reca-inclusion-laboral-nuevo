...
- `2026-04-16` - finalización de formularios largos: hardening local de confirmación resiliente aplicado sobre `Presentación`, `Sensibilización`, `Condiciones de la Vacante`, `Selección`, `Contratación`, `Inducción Organizacional` e `Inducción Operativa`. Todas las routes de finalización ya persisten `payload_normalized.metadata.finalization` con `form_slug`, `request_hash`, `idempotency_key` e `identity_key`; además existe la route autenticada `POST /api/formularios/finalization-status` con recovery/backfill desde `form_finalization_requests` + `formatos_finalizados_il`. En cliente, el flujo compartido ahora detecta timeout de confirmación, entra en `verificando_publicacion`, hace polling sin re-publicar y expone `Verificar de nuevo` cuando la confirmación sigue sin resolverse. Validación local cerrada con `npx vitest run` focal (`50/50`), `npx playwright test e2e/finalization-controlled.spec.ts` (`5/5`), `npm run lint`, `npm run build` y `npm run spellcheck`.
