...
export async function POST(request: Request) {
  ...
  try {
    ...
    const parsed = contratacionFinalizeRequestSchema.safeParse(body);
    ...
    try {
      ...
      console.error("[contratacion.raw_payload_upload] failed", {
    ...
    console.error("[contratacion.finalize] failed", error);
    return NextResponse.json(
      buildFinalizationRouteErrorBody({
        stage: finalizationStage,
        error:
